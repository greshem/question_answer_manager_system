#open(FILE, "20160411haitian01_raw.txt") or die("import error \n");
do("common.pl");
...
sub print_wget($$)
{
    ...
    if(length($q)==0 ||  length($a)==0)
    {
        return ;
            ...
            print <<EOF
wget --post-data="question=$q&answer=$a&topic=$topic&user=haitian"  http://192.168.1.11/qa_shadow/question_modifyok.php  -O  output/$count
EOF
;
...
__DATA__
RETVAL=\$?
[ \$RETVAL -ne 0 ] && touch /tmp/haitian_$count

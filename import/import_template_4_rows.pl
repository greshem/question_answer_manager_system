...
sub print_wget($$)
{
    ...
    if(length($a)==0 || length($a)==0)
    {
        return ;
            ...
            print <<EOF
wget --post-data="question=$q&answer=$a&topic=$topic&user=internship"  http://192.168.1.11/qa_shadow/question_modifyok.php  -O  output/$count
EOF
;
